...
class Model {
  ...
  static bulkCreate(records, options = {}) {
    ...
    if (options.onConflictUpdate && dialect !== 'postgres') {
      return Promise.reject(new Error(`${dialect} does not support the onConflictUpdate option.`));
    }
    ...
    if (options.onConflictUpdate !== undefined && options.onConflictUpdate.update) {
      const where = options.onConflictUpdate.update.where;
      const columns = options.onConflictUpdate.update.columns;
      if (columns && _.isArray(columns) && columns.length) {
        options.onConflictUpdate.update.columns = _.intersection(
          _.without(Object.keys(this.tableAttributes), this._timestampAttributes.createdAt),
          columns
        );
      } else {
        return Promise.reject(new Error('onConflictUpdate.update.columns option only supports non-empty array.'));
      }
      if (where) {
        options.onConflictUpdate.update.where = Utils.mapWhereFieldNames(where, this.constructor);
      }
    }
    ...
    return Promise.try(() => {
      ...
      if (options.individualHooks) {
        ...
        if (options.updateOnConflict && options.updateOnConflict.update && options.updateOnConflict.update.columns) {
          options.updateOnConflict.update.columns = options.updateOnConflict.update.columns.map(attr => this.rawAttributes[attr].field || attr);
        }
